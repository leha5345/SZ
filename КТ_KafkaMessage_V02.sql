...
-- просмотры запросов контракта ЕИС по карточке тендера
-- для просмотра запросов в переменную @TENDERTABLEID указать номер карточки тендера

DECLARE @TENDERTABLEID nvarchar(20),@REFTABLEIDSOURCE int, @REGNUMBER nvarchar(19) ;
	SET @REFTABLEIDSOURCE = 30968 -- ничего не менять, 30968 это таблица тендеров тендеры
	SET @TENDERTABLEID = '070732_268' -- сюда указать номер КТ
...
-- Посмотреть есть ли сопоставление ЕИС контракта с карточкой тендера
select EISContractPackage.COMPANYSOURCE
	...
	,EISContractPackage.TENDERTABLEID -- если тут пусто значит сопоставления с ЕИС контрактом нету 
	,EISContract.TENDERTABLEID -- если тут пусто то с версиями контракта ЕИС нету
...
--	Out.decoded_value,
	'////' as Разделитель, -- справа от этого столбца должен отобразиться ответ на запрос
	Bxx.RECID as BxxRECID,
	Bxx.Direction,
	Bxx.KafkaRecordStatus,
	Bxx.CREATEDDATETIME,
--	Bxx.REFTRANSACTIONID,
	Bxx.decoded_value		-- если в этом поле нечитаемые символы, то класс поиска в ЕИС не нашел контракт
